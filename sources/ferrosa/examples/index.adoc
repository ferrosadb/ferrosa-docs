...
link:arrow-flight.html[*Arrow Flight Query Endpoint*] _New!_::
Read and write query results as columnar Apache Arrow over a gRPC Flight endpoint -- bearer-token auth, paged DoGet, DoPut/DoExchange ingest, and per-token-range endpoints for parallel reads. Pairs with the Python `SUBSCRIBE` driver for live change streams.
